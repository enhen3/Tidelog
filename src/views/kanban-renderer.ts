...
export class KanbanRenderer {
    ...
    private async renderPyramidMonth(panel: HTMLElement): Promise<void> {
        ...
        const monthLabel = getLanguage() === 'en' ? targetMonth.format('MMM') : targetMonth.format('M月');
        ...
        titleArea.createSpan({ cls: 'tl-pyramid-layer-icon', text: '🏔️' });
        titleArea.createSpan({ cls: 'tl-pyramid-layer-title', text: t('kanban.monthGoals', monthLabel) });
        ...
        if (h.kanbanMonthOffset !== 0) {
            const resetBtn = header.createEl('button', { cls: 'tl-pyramid-today-btn', text: t('kanban.thisMonth') });
        ...
        if (!file) {
            layer.createDiv({ cls: 'tl-pyramid-empty', text: t('kanban.noMonthPlan') });
            return;
        }
        ...
        const goalSections = [t('kanban.monthlyGoals'), t('kanban.milestones'), '月度目标', '关键里程碑'];
        ...
        if (goals.length > 0) {
            ...
            progressWrap.createSpan({ cls: 'tl-pyramid-progress-label', text: t('kanban.completed', String(doneCount), String(goals.length)) });
        }

        // Task list body (same pattern as weekly/daily)
        const body = layer.createDiv('tl-pyramid-month-detail');
        if (goals.length === 0) {
            body.createDiv({ cls: 'tl-pyramid-empty', text: t('kanban.noMonthGoals') });
        } else {
            for (const goal of goals) {
                ...
                const label = card.createSpan({ cls: 'tl-pyramid-task-text', text: goal.text });
    ...
    private async renderPyramidWeek(panel: HTMLElement, targetDate: ReturnType<typeof moment>, weekStart: ReturnType<typeof moment>): Promise<void> {
        ...
        const weekLabel = getLanguage() === 'en'
            ? `${weekStart.format('MMM D')} – ${weekEnd.format('MMM D')} W${weekNum} goals`
            : `${weekStart.format('M月D日')}～${weekEnd.format('M月D日')} 第${weekNum}周目标`;
        ...
        const titleArea = header.createDiv('tl-pyramid-week-title-area');
        titleArea.createSpan({ cls: 'tl-pyramid-layer-icon', text: '📅' });
        titleArea.createSpan({ cls: 'tl-pyramid-layer-title', text: weekLabel });
        titleArea.addClass('tl-clickable');
        ...
        if (h.kanbanWeekOffset !== 0) {
            const todayBtn = header.createEl('button', { cls: 'tl-pyramid-today-btn', text: t('kanban.thisWeek') });
        ...
        if (!file) {
            body.createDiv({ cls: 'tl-pyramid-empty', text: t('kanban.noWeekPlan') });
            return;
        }
        ...
        const weekSections = [t('kanban.keyTasks'), t('kanban.weekGoals'), t('kanban.weekFocus'), '关键任务', '本周目标', '本周重点'];
        const keyItems = allItems.filter(i => weekSections.some(s => i.section.includes(s)));

        if (keyItems.length === 0) {
            body.createDiv({ cls: 'tl-pyramid-empty', text: t('kanban.noWeekTasks') });
        } else {
            ...
            if (keyItems.length > 0) {
                ...
                progressWrap.createSpan({ cls: 'tl-pyramid-progress-label', text: t('kanban.completed', String(doneCount), String(keyItems.length)) });
            }
            ...
            for (const item of keyItems) {
                ...
                const label = card.createSpan({ cls: 'tl-pyramid-task-text', text: item.text });
    ...
    private async renderPyramidDaily(panel: HTMLElement): Promise<void> {
        ...
        const DAY_LABELS = getLanguage() === 'en'
            ? ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
            : ['周一', '周二', '周三', '周四', '周五', '周六', '周日'];
        const targetDay = moment().add(h.kanbanDayOffset, 'days');
        const dayIdx = targetDay.isoWeekday() - 1;
        const dayLabel = getLanguage() === 'en'
            ? `${targetDay.format('MMM D')} ${DAY_LABELS[dayIdx]} tasks`
            : `${targetDay.format('M月D日')} ${DAY_LABELS[dayIdx]}任务`;
        ...
        const titleArea = header.createDiv('tl-pyramid-week-title-area');
        titleArea.createSpan({ cls: 'tl-pyramid-layer-icon', text: '📋' });
        titleArea.createSpan({ cls: 'tl-pyramid-layer-title', text: dayLabel });
        titleArea.addClass('tl-clickable');
        ...
        if (!isToday) {
            const resetBtn = header.createEl('button', { cls: 'tl-pyramid-today-btn', text: t('kanban.today') });
        ...
        if (tasks.length > 0) {
            ...
            progressWrap.createSpan({ cls: 'tl-pyramid-progress-label', text: t('kanban.completed', String(doneCount), String(tasks.length)) });
        }
        ...
        if (isToday) {
            try {
                ...
                if (carryTasks.length > 0) {
                    ...
                    carryHeader.createSpan({ text: t('kanban.carryForward', String(carryTasks.length)) });

                    for (const ct of carryTasks) {
                        const row = carrySection.createDiv('tl-carry-forward-row');
                        row.createSpan({ cls: 'tl-carry-forward-text', text: ct.text });
                        row.createSpan({ cls: 'tl-carry-forward-date', text: ct.date.substring(5) });
        ...
        if (tasks.length === 0) {
            body.createDiv({ cls: 'tl-pyramid-empty', text: t('kanban.noTasks') });
        } else {
            ...
            if (inProgress.length > 0) {
                ...
                section.createDiv({ cls: 'tl-task-group-header', text: t('kanban.inProgress', String(inProgress.length)) });
            ...
            if (completed.length > 0) {
                ...
                doneHeader.setText(t('kanban.completedSection', String(completed.length)));
    ...
    private renderTaskCard(container: HTMLElement, task: { text: string; done: boolean; indent: number }, filePath: string): void {
        ...
        const label = card.createSpan({ cls: 'tl-pyramid-task-text', text: task.text });
