...
> [!warning] 这是一套演示数据
> Vault 中的人物、计划与复盘均为虚构示例，不包含真实用户隐私或真实 License。
...
> [!tip] 第一次打开
> 若 Obsidian 显示第三方插件安全提示，请确认信任此 Vault。生成脚本会提供 7 天的本地演示 Pro 状态，但不包含真实 License Key；需要现场重新生成 AI 内容时，请确认网络畅通并查看当前配额。`;
}
...
> [!tl-meta] 体验目标
> 跟随产品经理林澈推进一次跨团队 Beta 上线。无需调整 AI 设置，先看懂 TideLog 如何把一条记录变成下一步。
...
> [!tip] AI 功能
> 演示 Vault 已放好生成后的示例结果。现场调用 AI 时无需配置；若无法生成，请先检查网络和当前配额。`],
...
这意味着 AI 不是在没有提示时扫描整个 Vault。只有用户主动对话、生成/更新报告，或完成复盘后刷新建议时才会发起相应调用。

## 长生成的可靠性

- TideLog 的内置 AI 服务会优先使用 SSE 流式响应。
- 流式响应异常时，会显示明确的失败提示，方便稍后重试。
...
4. 测试时注明设备、Obsidian 版本和文件数量。

## 隐私边界

- 演示 Vault 不含真实用户数据或真实 License；本地演示 Pro 状态只有 7 天有效期。
- TideLog 不包含客户端遥测、分析 SDK、动态广告或自动更新机制。
- License 在支持 SecretStorage 的 Obsidian 版本中通过 SecretStorage 保存。
...
这是一套虚构演示数据，不包含真实用户隐私或真实 License。`);
}
